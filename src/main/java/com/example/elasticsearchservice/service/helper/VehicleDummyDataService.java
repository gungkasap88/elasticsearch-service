package com.example.elasticsearchservice.service.helper;
...
public class VehicleDummyDataService {
    ...
    private static VehicleResponChannel buildVehicle(final String id,
                                        final String name,
                                        final String number,
                                        final String date) {
        VehicleResponChannel vehicleResponChannel = new VehicleResponChannel();
        vehicleResponChannel.setId(id);
        vehicleResponChannel.setName(name);
        vehicleResponChannel.setNumber(number);
        try {
            vehicleResponChannel.setCreated(DATE_FORMAT.parse(date));
        } catch (ParseException e) {
            LOG.error(e.getMessage(), e);
        }

        return vehicleResponChannel;
    }
}
